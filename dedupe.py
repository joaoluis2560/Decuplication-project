...
class DedupeModel (object):
    ...
    def remove_space(self,lista):
        ...
        s = [re.sub(r'(?<=[A-Z]{3}[0-9]{1})\s(?=[A-Z]{2})','', i) for i in s]
        s = [re.sub(r'(?<=[A-Z])\s+(?=\d+)','', i) for i in s]
    ...
    def define_tokens(self, tokens):
        ...
        df.columns = ['id_familia', 'produto', 'id']
        cols = list(df.columns)
        cols = [cols[-1]] + cols[:-1]
        df = df[cols]
        df['id_familia'] = [','.join(map(str, l)) for l in df['id_familia']]
        return df
    
    def stop_words(self,df):
        stop = stopwords.words('portuguese')
        df['produto'] = df['produto'].apply(lambda x: ' '.join([word for word in x.split() if word not in (stop)]))
        return df
    ...
    def abbreviation (self, df_):
        ...
        df_ = df_.rename(columns={"id": "internalId"})
        return df_
    ...
    def parse_df (self, df_, testes):
        ...
        df_compare = df_compare.groupby('id', group_keys=False).apply(lambda x: x.loc[x.Fuzzy.idxmax()])
        df_compare = df_compare[['id','Label']]
        testes_compare = testes[['id']]
        df = pd.concat([df_compare, testes_compare]) 
        df = df['id'].drop_duplicates(keep=False)
        df_new_product = df.to_frame()
        df_new_product['Label'] = 'New_product'
        df_final  = df_compare.append(df_new_product)
        x= df_final.set_index('id')
        df1 = x.reindex(index=testes['id'])
        df_final = df1.reset_index()
        df_final= df_final.merge(testes, on='id', how='inner')
        df_final=  df_final.rename(columns={"produto": "displayName"})
        x = df_.append(df_final, ignore_index = False)
        df_classe = x.drop_duplicates(subset=['id'])
    ...
    def output(self,df):
        ...
        for index, row in df.iterrows():
            q = row['id'], row['displayName'],row['id_familia'], row['Similairity'],row['Similarity_tokens'], row['id_base'], row['displayName_base']
            products['products'].append({
            'index': index,
            'id': row['id'],
    ...
    def predict(self, file=False):
        ...
        if verify == True:
            ...
            matches = DedupeModel.awesome_cossim_top(self,tfidf_matrix_train, tfidf_matrix_test_id.transpose(),1,0.3) 
        if matches.nnz == 0:
        ...
        if teste == True:
            ...
            df_output = DedupeModel.parse_df(self, threashold, query) 
