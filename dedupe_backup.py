...
class DedupeModel (object):
    ...
    def busca_padrao_p1 (self, lista): 
        regex_tv =  r'(\S*\d\d+\S*)|(\d\S\S\S+\d*)|(\S*\d+\S*)'
        tokens2 ={}
        tokens ={}
    ...
    def awesome_cossim_top(self,tfidf_matrix_train, tfidf_matrix_test_id, ntop, lower_bound=0):
        ...
        return  True if x.get_shape()==0 else x
    ...
    def validate_tokens(self, df): 
        df['Similarity_tokens'] = np.where((df['id_familia_base'] == "") & (df['id_familia'] == "")
                     ,'s/t', df.apply(partial(self.apply_sm, c1='id_familia_base', c2='id_familia'), axis=1))
        return df
    
    def threashold (self, df): 
       df.loc[((df['Similairity'] >= 0.9)& (df['Fuzzy'] >=90))  & (df['Similarity_tokens'] >'0.99'), 'Label'] = 'dedupe' 
       df.loc[((df['Similairity'] >= 0.9)&(df['Fuzzy'] >=90))  & (df['Similarity_tokens'] <'0.99'), 'Label'] = 'Human Validation' 
       df.loc[((df['Similairity'] >= 0.9)| (df['Fuzzy'] >=90))  & (df['Similarity_tokens'] >'0.99'), 'Label'] = 'dedupe'
       df.loc[((df['Similairity'] >= 0.9)| (df['Fuzzy'] >=90))  & (df['Similarity_tokens'] >'0.8'), 'Label'] =  'Human Validation'
       df.loc[((df['Similairity'] >= 0.9)| (df['Fuzzy'] >=90))  & (df['Similarity_tokens'] <'0.8'), 'Label'] =  'New_product'
       df.loc[((df['Similairity'] < 0.9) & (df['Fuzzy'] < 90))  & (df['Similarity_tokens'] >'0.99'), 'Label'] = 'Human Validation'
       df.loc[((df['Similairity'] < 0.9) & (df['Fuzzy'] < 90))  & (df['Similarity_tokens'] <'0.99'), 'Label'] = 'New_product'
       return df

    def parse_df (self, df_, testes):
        df_compare = df_.loc[(df_['Label'] != 'New_product')]
        df_compare = df_compare.drop_duplicates(subset = ["internalId"])
    ...
    def predict(self, file=False):
        ...
        abr = DedupeModel.abbreviation(self, stop) 
        learning_df = DedupeModel.read_dedupe(self) 
        df_b = DedupeModel.deduplication_learning (self, stop, learning_df) 
        ...
        if verify == True:
            ...
            matches = DedupeModel.awesome_cossim_top(self,tfidf_matrix_train, tfidf_matrix_test_id.transpose(),1,0.5) 
        if matches.nnz == True:
        ...
        if teste == True:
            ...
            validation = DedupeModel.validate_tokens(self,fuzzy)
